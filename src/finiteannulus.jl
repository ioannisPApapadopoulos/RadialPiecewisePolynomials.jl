struct FiniteContinuousZernikeAnnulusMode{T, N<:Int, P<:AbstractVector, M<:Int, J<:Int} <: Basis{T}
    N::N
    points::P
    m::M
    j::J
...
function FiniteContinuousZernikeAnnulusMode{T}(N::Int, points::AbstractVector, m::Int, j::Int) where {T}
    @assert length(points) > 1 && points == sort(points)
    @assert m ≥ 0
    @assert m == 0 ? j == 1 : 0 ≤ j ≤ 1
    FiniteContinuousZernikeAnnulusMode{T, Int, typeof(points), Int, Int}(N, points, m, j)
end
FiniteContinuousZernikeAnnulusMode(N::Int, points::AbstractVector, m::Int, j::Int) = FiniteContinuousZernikeAnnulusMode{Float64}(N, points, m, j)
...
==(P::FiniteContinuousZernikeAnnulusMode, Q::FiniteContinuousZernikeAnnulusMode) = P.N == Q.N && P.points == Q.points && P.m == Q.m && P.j == Q.j

function _getCs(points::AbstractVector{T}, m::Int, j::Int) where T
    K = length(points)-1
    [ContinuousZernikeAnnulusElementMode{T}([points[k]; points[k+1]], m, j) for k in 1:K]
end
